...
public class SecurityUserResource {
    ...
    public ResponseEntity<SecurityUserDTO> createSecurityUser(@Valid @RequestBody SecurityUserDTO securityUserDTO)
        ...
        Optional<SecurityUser> findOneByLogin = securityUserRepository.findOneByLogin(securityUserDTO.getLogin());
        if (!findOneByLogin.isEmpty()) {
            throw new BadRequestAlertException("User name is already exists ", ENTITY_NAME, "idexists");
        }
        Optional<SecurityUser> findOneByEmail = securityUserRepository.findOneByEmailIgnoreCase(securityUserDTO.getEmail());
