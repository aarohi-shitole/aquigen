package com.techvg.inventory.schedular;
...
public class ScheduledTasks {
    ...
    public void reportCurrentTime() {
        log.info("Schedular Quotation Reminder", dateFormat.format(new Date()));
    ...
    private void checkPurchaseQuotation() {
        ...
        for (PurchaseQuotationDTO purchaseQuotationDTO : page.getContent()) {
            ...
            if (diff > 3) {
                ...
                notificationDTO.setMassage(
                    diff + " days gone for this quotation given by you to client " + purchaseQuotationDTO.getClientDetails().getClientName()
                );
                notificationDTO.setTitle("Quotation Reminder for " + purchaseQuotationDTO.getId());
