...
export class CategoriesService {
  ...
  update(categories: ICategories): Observable<EntityResponseType> {
    return this.http.put<ICategories>(`${this.resourceUrl}/${getCategoriesIdentifier(categories) as number}`, categories, {
      observe: 'response',
    });
  }

  partialUpdate(categories: ICategories): Observable<EntityResponseType> {
    return this.http.patch<ICategories>(`${this.resourceUrl}/${getCategoriesIdentifier(categories) as number}`, categories, {
      observe: 'response',
    });
  }
